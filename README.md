# RNA-Seq Analysis (MSc Projects)

1. RNA-Seq analysis of Venetoclax-resistant mantle cell lymphoma cell line  

- The data analysed as part of this exercise comes from Zhao et al. 2019 (10.1016/j.ccell.2019.04.005); raw data can be downloaded here:   https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE116132.
- As part of this project, derived/resistant cell line J4 was compared to parental cell line to investigate the mechanisms of Venetoclax resistance
- QC, trimming, and quantification were performed on a university HPC cluster with the use of fastQC, TrimGalore, and salmon respectively. Resulting .sf files are included in Files1 (derived samples), and Files2 (parental samples) and are ready for downstream analysis. Part of bash script used to create them is also included as an example.


